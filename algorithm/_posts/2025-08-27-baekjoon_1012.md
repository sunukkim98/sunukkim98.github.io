...
- 각 테스트 케이스에서 m(배추밭의 가로길이), n(배추밭의 세로길이), k(배추가 심어져 있는 위치의 개수)를 입력받는다.
- 가로의 길이가 m이고 세로의 길이가 n인 모두 0으로 초기화된 그래프 초기화
- k번의 반복을 통해 배추가 있는 좌표인 x, y를 입력받고 해당 위치의 값을 1로 변경한다.
- 모든 노드에 대해서 dfs() 함수를 통해 서로 인접해있는 배추들이 몇 군데 있는지 탐색한다.
- 모든 노드에 대해 탐색 완료후 서로 인접해있는 배추들이 몇 군데인지 출력(cnt 출력)
...
def dfs(x, y):
    ...
    for i in range(4):
        ...
        if (0 <= nx < m) and (0 <= ny < n) and graph[ny][nx] == 1:
            ...
            dfs(nx, ny)
```
- dx, dy는 상하좌우 네 방향을 반복하기 위한 리스트
- 상하좌우 방향을 반복
- nx, ny는 현제 방향에서 다음으로 탐색할 x, y좌표
- 만약 nx와 ny가 유효한 범위이고, 값이 1이라면 -1로 변경
- 다음으로 탐색할 위치 반복해 진행
